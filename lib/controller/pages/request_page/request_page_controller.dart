...
class RequestPageController extends BaseController {
    ...
    RequestPageController(this.requestId, this.studentId, this.teacher, this.lessonDate);
    ...
    Future<void> initTeacher() async {
        final foundTeacher = await dataManager.database.getTeacher(request?.teacherId ?? '');
    ...
    Future<void> initLessonDate() async {
        final foundDate = await dataManager.database.getLessonDate(request?.lessonDateId ?? '');
        if (foundDate == null) {
    ...
    String get date => '${lessonDate?.weekday ?? ''}, ${lessonDate?.hourTime ?? ''}';
    ...
    Future<void> sendRequest(BuildContext context) async {
        request = Request.noKey(lessonDate?.lessonDateId ?? '', teacher?.userId ?? '', studentId ?? '', 0);
        ...
        await dataManager.database.addRequestIdToTeacher(teacher?.userId ?? '', requestKey);
        await dataManager.database.addRequestIdToStudent(studentId ?? '', requestKey);
