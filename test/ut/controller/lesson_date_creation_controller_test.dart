...
void main() {
  ...
  group('Date and time', () {
    setUp(() {
      lessonDateCreationController = LessonDateCreationPageController(createTeacher(), () {});
    });
    test('Initial date and time are default', () {
      expect(lessonDateCreationController.date, DateFormatter.onlyDateString(DateTime.now()));
      expect(lessonDateCreationController.time, DateFormatter.timeString(const TimeOfDay(hour: 12, minute: 0)));
    });
    ...
    test('Set new date. setPossibleNewDate should set new date', () {
      ...
      expect(lessonDateCreationController.date, DateFormatter.onlyDateString(newDateTime));
    });
    ...
    test('Set new time same as old. setPossibleNewDate should not set new time', () {
      ...
      expect(lessonDateCreationController.time, DateFormatter.timeString(newTimeOfDay));
    });
  ...
  group('Price and duration', () {
    ...
    test('Validate incorrect duration', () {
      expect(lessonDateCreationController.validateDuration('-20'), 'Duration cannot be less than 10');
    });

    test('Validate too large duration', () {
      expect(lessonDateCreationController.validateDuration('1230'), 'Duration cannot be greater than 1000');
    });
    ...
    test('Validate empty price', () {
      expect(lessonDateCreationController.validatePrice(''), 'Price cannot be null');
    });

    test('Validate negative price', () {
      expect(lessonDateCreationController.validatePrice('-10'), 'Price cannot be negative');
    });
